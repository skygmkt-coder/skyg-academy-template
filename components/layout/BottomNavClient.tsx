...
function getItems(user: NavUser): PillItem[] {
  if (!user) {
    return [
      ...
      { href: "/registro", label: "Regístrate", icon: "plus", highlight: true },
    ];
  }
...
export default function BottomNavClient({ user }: { user: NavUser }) {
  ...
  return (
    ...
    >
      <div
        className="mobile-nav-shell flex items-center px-2 py-2 gap-0.5"
        style={{
          ...
          if (item.isAction) {
            return (
              <button
                key="action"
                onClick={logout}
                className="mobile-nav-item flex flex-col items-center justify-center gap-1"
                style={{
                  minWidth: 58,
                  padding: "9px 7px",
                  borderRadius: "99px",
