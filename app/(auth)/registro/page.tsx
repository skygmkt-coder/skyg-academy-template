...
export default function RegistroPage({ searchParams }: { searchParams: { error?: string } }) {
  return (
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="glass rounded-2xl p-8 border border-white/10 shadow-card">
          ...
          <form action={signUp} className="space-y-5">
            <div>
              <label className="text-sm text-muted mb-2 block">Nombre completo</label>
              <input name="full_name" type="text" required placeholder="Tu nombre"
            ...
            <div>
              <label className="text-sm text-muted mb-2 block">Correo electrónico</label>
              <input name="email" type="email" required placeholder="tu@correo.com"
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-muted/50 focus:outline-none focus:border-primary/50 transition-colors text-sm" />
            </div>
            <div>
              <label className="text-sm text-muted mb-2 block">Contraseña</label>
