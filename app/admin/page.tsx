...
export default async function AdminDashboard() {
  ...
  const [
    ...
    supabase
      .from("courses")
      .select("id,title,published,scheduled_at,created_at")
      .order("created_at", { ascending: false })
      .limit(5),
    supabase
      .from("live_classes")
      .select("id,title,scheduled_at,zoom_url")
  ...
  const totalRevenue =
    orders?.reduce((sum, o) => sum + (o.amount_cents || 0), 0) || 0;

  const stats = [
    {
      label: "Cursos totales",
      value: totalCourses || 0,
      sub: `${publishedCourses || 0} publicados`,
      icon: "📚",
      color: "rgba(53,137,242,0.12)",
          ...
          if (s.href) {
            return (
              <Link
                key={s.label}
                href={s.href}
                className="rounded-2xl p-5 border block hover:scale-[1.01] transition-all"
          ...
          <div className="glass rounded-2xl border border-white/5 overflow-hidden">
            {(!upcomingClasses || upcomingClasses.length === 0) && (
              <div className="px-5 py-8 text-center text-sm text-white/30">
                ...
                <Link
                  href="/admin/clases-en-vivo/nuevo"
                  className="text-primary hover:underline"
                >
                  Crear una
                </Link>
