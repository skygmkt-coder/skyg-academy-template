...
export async function addModule(courseId: string, formData: FormData) {
  const supabase = createClient();
  const { data: last } = await supabase.from("modules").select("order_index").eq("course_id", courseId).order("order_index", { ascending: false }).limit(1).single();
  await supabase.from("modules").insert({ course_id: courseId, title: formData.get("title"), order_index: (last?.order_index ?? -1) + 1 });
  revalidatePath(`/admin/cursos/${courseId}`);
}

export async function addLesson(moduleId: string, courseId: string, formData: FormData) {
  const supabase = createClient();
  const { data: last } = await supabase.from("lessons").select("order_index").eq("module_id", moduleId).order("order_index", { ascending: false }).limit(1).single();
