...
export default async function UsuariosPage() {
  ...
  const visibleProfiles = profiles?.filter((p) => !p.is_super_admin) || [];
  ...
  const getRoleBadge = (p: any) => {
    if (p.is_admin) {
      return {
        label: "🛡 Admin",
        bg: "rgba(53,137,242,0.12)",
        color: "#60a5fa",
        border: "rgba(53,137,242,0.25)",
      };
    }
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-white/40 mt-1">
            {visibleProfiles.length} usuarios registrados
          </p>
        </div>
      </div>
      ...
      <div className="glass rounded-2xl border border-white/5 overflow-hidden">
        <div className="grid grid-cols-[1fr_auto_auto_auto] gap-4 px-5 py-3 border-b border-white/5">
          ...
          <span className="text-[10px] font-bold text-white/30 uppercase tracking-wider w-24 text-center">
            Cursos
          </span>
          <span className="text-[10px] font-bold text-white/30 uppercase tracking-wider w-28 text-center">
            Rol
          </span>
          <span className="text-[10px] font-bold text-white/30 uppercase tracking-wider w-16 text-center">
            Acción
          </span>
        </div>
